<?php
/**
 *	SIDEBAR - Innovation (100 Years)
 *
 **/


?>
<div class="bbg-innovation">

	<h2>Over <strong>100 Years</strong> of Innovation</h2>

	<div class="innov-parts">

		<div class="innov-sub-parts">
			<p>The Jacuzzi<sup>&reg;</sup> Difference</p>
			<ul>
				<?php 
				//Replacing the theme folder function with static link. And giving dimension to image 
				//echo do_shortcode('[video_lightbox_youtube video_id="qxYknzV-yNQ" width="640" height="480" anchor="'.get_template_directory_uri().'/images/brochure/left1.png"]'); ?>
				<a id="" rel="wp-video-lightbox" href="https://www.youtube.com/watch?v=qxYknzV-yNQ&amp;width=640&amp;height=480" title=""><img src="http://www.jacuzzi.com/hot-tubs/wp-content/themes/jht/images/brochure/left1.png" class="video_lightbox_anchor_image" alt="" width="128" height="114"></a>
				<!--img src="<?php echo get_template_directory_uri(); ?>/images/brochure/left1.png" width="128" height="114" align="left"/-->
				<li>50 years of hot tub innovation</li>
				<li>The name that started an industry</li>
				<li>Luxury features and upgrades</li>
			</ul>
			<a href="http://www.jacuzzi.com/hot-tubs/about/">Why Jacuzzi</a>
		</div>
		
		<div class="innov-sub-parts">
			<p>Advanced Hydrotherapy</p>
			<ul>
				<img src="http://www.jacuzzi.com/hot-tubs/wp-content/themes/jht/images/brochure/left2.png" width="128" height="114" align="left"/>
				<li>Relaxation and stress relief</li>
				<li>Speed recovery of injuries</li>
				<li>Promote flow of endorphines</li>
			</ul>
			<a href="http://www.jacuzzi.com/hot-tubs/hydrotherapy/">Why Hydrotherapy Works</a>
		</div>
		
		<div class="innov-sub-parts">
			<p>Industry-Leading 10-Year Warranty</p>
			<ul>
				<img src="http://www.jacuzzi.com/hot-tubs/wp-content/themes/jht/images/brochure/left3.png" width="128" height="114" align="left"/>
				<li>Carefree ownership</li>
				<li>Easy water care</li>
				<li>Quality materials</li>
			</ul>
			<a href="http://www.jacuzzi.com/hot-tubs/warranty-options/">Warranty Options</a>
		</div>

		<div class="clearfix"><p>&nbsp;</p></div>
	
	</div>
</div>